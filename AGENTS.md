...
`src/bin/esp32temp.rs` is the firmware entrypoint. Shared logic lives in `src/` modules such as `wifi.rs`, `mqtt.rs`, `esphome_api.rs`, `apiserver.rs`, `measure.rs`, `state.rs`, `config.rs`, and `rmt_ow.rs`, with `src/lib.rs` wiring exports.

UI assets are embedded into the firmware: `templates/index.html.ask` (Askama template) and the files in `static/` (`form.js`, `index.css`, `favicon.ico`), which `build.rs` gzip-compresses before embedding. Build/runtime configuration files are at the repo root (`sdkconfig.defaults`, `partitions.csv`, `rust-toolchain.toml`) plus `.cargo/config.toml` for the default ESP32-C3 target and ESP-IDF version. ESP-IDF component lockfiles for native `onewire_bus` integration are `components_esp32c3.lock` and `components_esp32.lock`. Helper scripts include `flash_c3`, `flash_wroom32`, `make_ota_image_c3`, and `make_ota_image_wroom32`.
...
Keep modules focused by subsystem (WiFi, MQTT, ESPHome API, HTTP API, measurement). Prefer explicit error propagation (`anyhow::Result`) and keep embedded-specific constants near the hardware logic that uses them.
...
PRs should include: what changed, why, how it was tested (commands + device/manual checks), and any required target/feature setup (ESP32 vs ESP32-C3). Add screenshots only for web UI changes, and mention OTA image output names (`firmware-c3.bin` or `firmware-wroom32.bin`) when changing release/flash flows.
